...
pub async fn run(args: &EcosystemArgs, context: &Context) -> Result<()> {
    ...
    if contracts_exist(&state_manager).await? {
    ...
    // Load and display chain information if --chain is provided
    if let Some(ref chain_name) = args.chain {
        display_chain_info(&state_manager, chain_name).await?;
    }
...
async fn display_chain_info(state_manager: &StateManager, chain_name: &str) -> Result<()> {
